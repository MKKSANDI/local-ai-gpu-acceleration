...
def load_tensor_plan(path: Path, selected_types: set[str]) -> list[dict[str, object]]:
    plan = json.loads(path.read_text(encoding="utf-8"))
    tensors = plan.get("tensors")
    if not isinstance(tensors, list):
        raise ValueError(f"{path} does not contain tensor rows")
    return [tensor for tensor in tensors if str(tensor.get("type")) in selected_types]
...
def select_tensors(
    ...
    *,
    ...
    selected = tensors
    if requested:
        selected = [tensor for tensor in selected if str(tensor.get("name")) in requested]
    if layer is not None:
        selected = [
            tensor
            for tensor in selected
            if tensor_role(str(tensor.get("name")))[0] == layer
        ]
...
def iq_tensor_shape(tensor: dict[str, object]) -> tuple[int, int]:
    tensor_type = str(tensor.get("type"))
    fmt = BLOCK_FORMATS.get(tensor_type)
    if fmt is None or tensor_type not in SUPPORTED_TYPES:
        raise ValueError(f"{tensor.get('name')} has unsupported IQ type: {tensor_type}")
...
def read_tensor_bytes(model: Path, tensor: dict[str, object]) -> bytes:
    tensor_type = str(tensor["type"])
    expected = expected_data_bytes(tensor_type, tensor.get("dimensions"))
    if expected is None:
        raise ValueError(f"{tensor['name']} has unsupported tensor type {tensor_type}")
    span = int(tensor.get("span_bytes") or 0)
    if span < expected:
        raise ValueError(f"{tensor['name']} span is smaller than expected IQ bytes")
...
def pack_tensor(*, model: Path, tensor: dict[str, object], out_dir: Path) -> dict[str, object]:
    values, block_count = iq_tensor_shape(tensor)
    tensor_type = str(tensor["type"])
    fmt = BLOCK_FORMATS[tensor_type]
    payload = read_tensor_bytes(model, tensor)
    label = sanitize_label(str(tensor["name"]))
    payload_path = out_dir / f"{label}.{tensor_type.lower()}_raw_blocks.bin"
    payload_path.write_bytes(payload)
    layer, role = tensor_role(str(tensor["name"]))
    kernel_label = {
        "IQ2_S": "iq2s_raw_block_probe",
        "IQ3_S": "iq3s_raw_block_probe",
    }[tensor_type]
    return {
        "name": tensor["name"],
        "type": tensor_type,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Pack target GGUF IQ tensors into raw resident block files."
    )
    parser.add_argument("--storage-root", default=None)
    parser.add_argument("--model", default=None)
    parser.add_argument("--tensor-plan", default=None)
    parser.add_argument("--output-dir", default=None)
    parser.add_argument("--type", action="append", choices=SUPPORTED_TYPES, default=None)
    ...
    selected_types = set(args.type or ["IQ2_S"])
    stamp = int(time.time())
    generated_at = datetime.now(timezone.utc).isoformat()
    type_label = "_".join(sorted(selected_types)).lower()
    out_dir = (
        Path(args.output_dir)
        if args.output_dir
        else root / "packs" / "iq" / model.stem / type_label / str(stamp)
    )
    out_dir.mkdir(parents=True, exist_ok=True)

    tensors = load_tensor_plan(tensor_plan, selected_types)
    tensors = select_tensors(
        tensors,
        requested=set(args.tensor) if args.tensor else None,
        layer=args.layer,
        limit=args.limit,
    )
    if not tensors:
        raise SystemExit("no IQ tensors selected")

    packed = [pack_tensor(model=model, tensor=tensor, out_dir=out_dir) for tensor in tensors]
    manifest = {
        "path": "iq_resident_pack",
